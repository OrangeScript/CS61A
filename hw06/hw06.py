    ...
    def vend(self):
        if(self.balance>=self.price and self.stock>0):
            if(self.balance==self.price):
                re= f'Here is your {self.name}.'
            else:
                re= f'Here is your {self.name} and ${self.balance-self.price} change.'
            self.stock-=1
            self.balance=0
            return re
...
def store_digits(n):
    ...
    # Build the linked list from most significant to least significant
    result = Link.empty
    for digit in digits:  # 反转数字顺序
...
def two_list(vals, counts):
    ...
    return Link(a,two_list(vals,counts))
