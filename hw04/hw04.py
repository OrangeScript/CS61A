...
def balanced(m):
    ...
    current_balanced = (total_mass(left_end) * length(left_arm) == 
                       total_mass(right_end) * length(right_arm))
    
    return current_balanced and balanced(left_end) and balanced(right_end)
...
def max_path_sum(t):
    ...
    this_max=label(t)
    for i in branches(t):
        max_in_branch=max_path_sum(i)
        if(max_in_branch+label(t)>this_max):
            this_max=max_in_branch+label(t)
