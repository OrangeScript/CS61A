...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def about_what(s):
        s_no_punc=remove_punctuation(s)
        s_no_punc=lower(s_no_punc)
        s_list=split(s_no_punc)
        for str in s_list:
            for i in subject:
                if(str==i):
                    return True
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    min_diff_word,min_diff=typed_word,limit
    for s in word_list:
         if(typed_word==s):
             return s
         diff=diff_function(typed_word,s,limit)
         if(diff<min_diff):
             min_diff=diff
             min_diff_word=s
         elif(diff==min_diff and min_diff_word==typed_word):
             min_diff_word=s
...
def feline_fixes(typed, source, limit):
    ...
    if(len(typed)==0 or len(source)==0):
        return max(len(typed),len(source))-min(len(typed),len(source))
    if(typed[0]==source[0]):
...
def minimum_mewtations(typed, source, limit):
    ...
    if(typed[0]==source[0]):
        ...
        min_operations = min(add, remove, substitute)
        return min_operations if min_operations <= limit else limit + 1
        # BEGIN
        "*** YOUR CODE HERE ***"
...
def time_per_word(words, timestamps_per_player):
    ...
    for s in timestamps_per_player:
        ...
        p.append(a)
    # p = [[s[i+1] - s[i] for i in range(len(s)-1)] for s in timestamps_per_player]
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    p=[[]  for _ in player_indices]
    for i in word_indices:
        min_time=time(match,0,i)
        this_player_min_time=0
        for j in player_indices:
            if(time(match,j,i)<min_time):
                min_time=time(match,j,i)
                this_player_min_time=j
        
        p[this_player_min_time].append(get_word(match,i))
    return p
