...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            other=place.ant
            assert self.can_contain(other) or other.can_contain(self), 'Too many ants in {0}'.format(place)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        instance=0
        while(instance<lower_bound):
            node=node.entrance
            instance+=1
            if(node.is_hive):
                return None
        
        while(instance<=upper_bound ):
            if(len(node.bees)!=0):
                return random_bee(node.bees)
            node=node.entrance
            instance+=1
            if(node.is_hive):
                return None
